...
ROOT = CWD + '/..'
__all__ = ['spectral', 'node2vec', 'svd', 'onehot', 'xavier']

def load_embedding(name, method):
    embedding = pickle.load(open(f'{ROOT}/embeddings/{method}_{name}.pt', 'rb'))
    print(f'Use cached {method} embedding')
    return embedding

def save_embedding(embedding, name, method):
    pickle.dump(embedding, open(f'{ROOT}/embeddings/{method}_{name}.pt', 'wb'))
...
def spectral(data, name, embedding_dim=64):
    try:
        ...
        print(f'embeddings/spectral_{name}.pt not found! Regenerating it now')
...
def node2vec(data, name, embedding_dim=64, epochs=20):
    try:
        ...
        print(f'embeddings/node2vec_{name}.pt not found! Regenerating it now')
...
def svd(data, name, embedding_dim=64):
    try:
        ...
        print(f'embeddings/svd_{name}.pt not found! Regenerating it now')
...
def onehot(data, name):
    try:
        ...
        print(f'embeddings/onehot_{name}.pt not found! Regenerating it now')
...
def xavier(data, name, embedding_dim=64):
    try:
        ...
        print(f'embeddings/xavier_{name}.pt not found! Regenerating it now')
